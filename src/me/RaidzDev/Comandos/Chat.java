...
public class Chat implements CommandExecutor, Listener
{
    ...
    public void mutetell(final PlayerCommandPreprocessEvent e) {
        final Player p = e.getPlayer();
        if (Chat.pausado && (e.getMessage().toLowerCase().startsWith("/tell") || e.getMessage().toLowerCase().startsWith("/msg") || e.getMessage().toLowerCase().startsWith("/bukkit:tell") || e.getMessage().toLowerCase().startsWith("/bukkit:msg"))) {
            e.setCancelled(true);
            p.sendMessage(String.valueOf(Config.prefix) + "§cChat pausado no momento!");
        }
    }
