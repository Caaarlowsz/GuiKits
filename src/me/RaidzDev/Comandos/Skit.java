	...
	public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
		...
		if (cmd.getName().equalsIgnoreCase("skit") && (p.hasPermission("guikits.skit") || p.isOp())) {
			if (args.length == 0) {
				...
				p.sendMessage(
						ChatColor.GRAY + "1 §7- Monte seu inventario da maneira como quer que os jogadores recebam");
			...
			if (args[0].equalsIgnoreCase("criar")) {
				if (args.length == 1) {
					...
					p.sendMessage(ChatColor.GRAY
							+ "1§7 - Monte seu inventario da maneira como quer que os jogadores recebam");
					p.sendMessage(ChatColor.GRAY + "2§7 - Digite /skit criar <nome>");
					return true;
				}
				...
				if (args.length <= 2) {
					...
					p.sendMessage(ChatColor.GRAY + "1§7 - Digite /skit aplicar <nome> <raio>");
					p.sendMessage(
							ChatColor.RED + "OBS:" + ChatColor.GRAY + " O kit precisa ter sido criado previamente!");
					return true;
				}
				final String name = args[1];
				...
				if (this.isInt(args[2])) {
					...
					Bukkit.getServer().broadcastMessage(String.valueOf(Config.cor) + "Kit " + name
							+ " aplicado para jogadores em um raio de " + numero + " blocos");
					p.sendMessage(String.valueOf(Config.cor) + "Kit " + name + " aplicado para jogadores em um raio de "
							+ numero + " blocos");
					return true;
				}
				return true;
