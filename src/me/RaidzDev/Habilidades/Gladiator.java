        ...
        if (event.getRightClicked() instanceof Player) {
            ...
            if (p.getItemInHand().getType() == Material.IRON_FENCE && KitAPI.getkit(p) == "Gladiator") {
                ...
                if (this.generateGlass) {
                    ...
                    r.sendMessage("§7Caso nao tenha nenhum vencedor depois de 4 minutos, voce voltara ao seu local de origem!");
                    ...
                    this.id2 = Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin)Main.instance, (Runnable)new Runnable() {
                        @Override
                        public void run() {
                    ...
                    this.id1 = Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin)Main.instance, (Runnable)new Runnable() {
                        ...
                        public void run() {
                            if (Gladiator.fighting.containsKey(p.getName()) && Gladiator.fighting.get(p.getName()).equalsIgnoreCase(r.getName()) && Gladiator.fighting.containsKey(r.getName()) && Gladiator.fighting.get(r.getName()).equalsIgnoreCase(p.getName())) {
                                ...
                                p.sendMessage("§7Nao houve nenhum vencedor, voce foi teleportado para o seu lugar de origem!");
                                r.sendMessage("§7Nao houve nenhum vencedor, voce foi teleportado para o seu lugar de origem!");
