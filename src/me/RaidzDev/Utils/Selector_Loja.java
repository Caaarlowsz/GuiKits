...
public class Selector_Loja implements Listener, CommandExecutor
{
    ...
    public void Warp(final PlayerInteractEvent e) {
        ...
        if ((e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) && p.getItemInHand().getType() == Material.EMERALD && p.getItemInHand().getItemMeta().getDisplayName().equalsIgnoreCase("§7» " + Config.cor + "Loja§7 «")) {
            p.chat("/loja");
        }
    }
    ...
    public void onPlayerCLickInventry(final InventoryClickEvent e) {
        ...
        if (e.getInventory().getTitle().equalsIgnoreCase(String.valueOf(Config.cor) + "Loja") && e.getCurrentItem() != null && e.getCurrentItem().getTypeId() != 0) {
            e.setCancelled(true);
