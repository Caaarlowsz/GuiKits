...
public class Info implements Listener
{
    ...
    public void onPlayerCLickInventry(final InventoryClickEvent e) {
        ...
        if (e.getInventory().getTitle().equalsIgnoreCase("§7Informa\u00e7\u00e3o") && e.getCurrentItem() != null && e.getCurrentItem().getTypeId() != 0) {
            e.setCancelled(true);
            if (e.getCurrentItem().getType() == Material.SKULL_ITEM) {
                e.setCancelled(true);
                p.closeInventory();
                p.sendMessage("§7Plugin Criado por " + Config.cor + "RaidzDev §7Nick do minecraft " + Config.cor + "RaidzDev§7.");
                return;
            }
