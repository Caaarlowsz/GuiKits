...
public class FeastHandler implements CommandExecutor {
	...
	public static void fillAllChests(final String s) {
		Chunk[] arrayOfChunk;
		for (int j = (arrayOfChunk = Main.instance.getServer().getWorld(s).getLoadedChunks()).length,
				i = 0; i < j; ++i) {
			...
			public void run() {
				...
				if (FeastHandler.time == 240) {
					Bukkit.broadcastMessage(
							String.valueOf(Config.prefix) + "Os Itens Do Feast Aparecer\u00e3o em 4 Minutos.");
				} else if (FeastHandler.time == 180) {
					Bukkit.broadcastMessage(
							String.valueOf(Config.prefix) + "Os Itens Do Feast Aparecer\u00e3o em 3 Minutos.");
				} else if (FeastHandler.time == 120) {
					Bukkit.broadcastMessage(
							String.valueOf(Config.prefix) + "Os Itens Do Feast Aparecer\u00e3o em 2 Minutos.");
				} else if (FeastHandler.time == 60) {
					Bukkit.broadcastMessage(
							String.valueOf(Config.prefix) + "Os Itens Do Feast Aparecer\u00e3o em 1 Minuto.");
				} else if (FeastHandler.time == 30) {
					Bukkit.broadcastMessage(
							String.valueOf(Config.prefix) + "Os Itens Do Feast Aparecer\u00e3o em 15 Segundos.");
				} else if (FeastHandler.time == 10) {
					Bukkit.broadcastMessage(
							String.valueOf(Config.prefix) + "Os Itens Do Feast Aparecer\u00e3o em 10 Segundos.");
				} else if (FeastHandler.time <= 5 && FeastHandler.time > 1) {
					FeastHandler.feastBroadcast(FeastHandler.time);
				} else if (FeastHandler.time == 1) {
					Bukkit.broadcastMessage(
							String.valueOf(Config.prefix) + "Os Itens Do Feast Aparecer\u00e3o em 1 Segundo.");
	...
	public static void feastBroadcast(final int i) {
		Bukkit.broadcastMessage(
				String.valueOf(Config.prefix) + "Os Itens Do Feast Aparecer\u00e3o em " + i + " Segundos.");
	}
