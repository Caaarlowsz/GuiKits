...
public class PlacaExtreme implements Listener
{
    ...
    public void inv(final PlayerInteractEvent e) {
        ...
        if (e.getAction() == Action.RIGHT_CLICK_BLOCK && e.getClickedBlock() != null && (e.getClickedBlock().getType() == Material.WALL_SIGN || e.getClickedBlock().getType() == Material.SIGN_POST)) {
            ...
            if (lines.length > 0 && lines[0].equals("§e---------") && lines.length > 1 && lines[1].equals("§eChallenge") && lines.length > 2 && lines[2].equals("§6Extreme") && lines.length > 3 && lines[3].equals("§e---------")) {
                Status.addMoney1(p, 100);
                ScoreBoard.onScoreboard(p);
                Bukkit.broadcastMessage(String.valueOf(Config.prefix) + " o jogador §c" + p.getName() + " §7completou o lava challenge extreme!");
