...
public class Placamedio implements Listener
{
    @EventHandler
    public void Escrever(final SignChangeEvent e) {
        final Player p = e.getPlayer();
        if (p.hasPermission("placa.medio") && e.getLine(0).equalsIgnoreCase("medio")) {
            e.setLine(0, "§e---------");
    ...
    public void inv(final PlayerInteractEvent e) {
        ...
        if (e.getAction() == Action.RIGHT_CLICK_BLOCK && e.getClickedBlock() != null && (e.getClickedBlock().getType() == Material.WALL_SIGN || e.getClickedBlock().getType() == Material.SIGN_POST)) {
            ...
            if (lines.length > 0 && lines[0].equals("§e---------") && lines.length > 1 && lines[1].equals("§eChallenge") && lines.length > 2 && lines[2].equals("§6Medio") && lines.length > 3 && lines[3].equals("§e---------")) {
                Status.addMoney1(p, 30);
                ScoreBoard.onScoreboard(p);
                p.sendMessage(String.valueOf(Config.prefix) + "Voc\u00ea completou a lava challenge §cmedio§7!");
